...
public class CollectionPreferencesEntry<T, C extends Collection<T>> extends StructuredPreferencesEntry<C> {
    ...
    public CollectionPreferencesEntry(Class<?> invoker, String name, Supplier<C> collectionSupplier, Function<String, T> transformer, Function<T, String> reverseTransformer) {
        super(
            invoker,
            name,
            structuredValue -> {
